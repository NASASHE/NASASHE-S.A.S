...
// ✅ BASE_URL para GitHub Pages / exe / local
const BASE = import.meta.env.BASE_URL

// 1. Lista de imágenes en /public/icons/
...
// ✅ helper: convierte "/icons/x.jpg" o "icons/x.jpg" al path correcto con BASE
const toPublicIconUrl = (path) => {
  if (!path) return ''
  // Si ya viene absoluto http(s), no lo tocamos
  ...
  // Si quedó empezando por "/", quítalo
...
function PaginaArticulos() {
  ...
  // La URL completa que guardaremos (ya con BASE)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await addDoc(collection(db, "articulos"), {
        ...
        stock: stockNum || 0,
        // ✅ guardamos la URL ya correcta para GH Pages
          ...
          <div>
            ...
            >
              ...
              {listaDeImagenes.map(imagen => (
                // ✅ value ahora incluye BASE_URL
                <option key={imagen} value={`${BASE}icons/${imagen}`}>
                  {imagen}
                </option>
              ))}
            </select>
                ...
                {editingId === articulo.id ? (
                  ...
                  <>
                    <td className="articulo-imagen-cell">
                      {articulo.imagenUrl && (
                        <img
                          src={toPublicIconUrl(articulo.imagenUrl)}
                          alt={articulo.nombre}
                        />
                      )}
                    </td>
